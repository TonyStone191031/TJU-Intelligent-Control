% In GHmodefuzzycontrol_v2, the fuzzy model "Ghfuzzy_v2.fis" is built with
% Ghfuzzy_v2_define.m. In order to use the Ghfuzzy_v2, you should run
% Ghfuzzy_v2_define.m first.
...
% Fuzzy Control
Ghfuzzy_v2 = readfis('Ghfuzzy_v2.fis');   % 导入模糊控制策略
Output_v2 = zeros(size(T_out,1),1);     % 模糊控制器的输出，有1个输出变量
...
for i = 2 : size(T_out,1)
    ...
    [Output_v2(i)] = evalfis(Ghfuzzy_v2, [T_in_pred_f(i), dT_out(i)]);
    Q_coef_f(i) = Output_v2(i);
