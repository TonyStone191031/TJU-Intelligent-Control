% This script uses GA to find the maximum value of Function B.
...
ezmesh('(sin(x)-1)^2 + 100*(sin(y)-(sin(x))^2)^2',[lbx,ubx,lby,uby],50);   %画出函数曲线
hold on;
%% 定义遗传算法参数
NIND=40;        %个体数目
MAXGEN=50;      %最大遗传代数
...
X=XY(:,1);Y=XY(:,2);
ObjV=(sin(X)-1).^2 + 100*(sin(Y)-(sin(X)).^2).^2;        %计算目标函数值
while gen<MAXGEN
   ...
   X=XY(:,1);Y=XY(:,2);
   ObjVSel=(sin(X)-1).^2 + 100*(sin(Y)-(sin(X)).^2).^2;             %计算子代的目标函数值
